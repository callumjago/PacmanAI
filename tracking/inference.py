...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            temp.update({self.getJailPosition():1.0})
        else:
            for pos in self.legalPositions:
                dist = util.manhattanDistance(pos, pacmanPosition)
                if emissionModel[dist] > 0:
                    temp[pos] = self.beliefs[pos]*emissionModel[dist]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            ...
            for p, probability in dist.items():
                temp[p] = temp[p] + probability*self.beliefs[pos]
        self.beliefs = temp
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                ...
                temp[p] = temp[p] + emissionModel[dist]*belief[p]
            if max(temp.values()) == 0:
                self.initializeUniformly(gameState)
            else:
                tempVals = []
                for i in range(self.numParticles):
                    tempVals.append(util.sample(temp))
                self.particles = tempVals
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            posDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            temp.append(util.sample(posDist))
        self.particles = temp
